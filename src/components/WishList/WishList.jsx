...
const WishList = ({wish}) => {
    const {bookName,author,image,tags,category,rating,yearOfPublishing,publisher,totalPages}=wish
    return (
        <div className="flex items-center shadow-xl m-8 p-8">
        ...
        <h1 className="text-2xl font-bold">{bookName}</h1>
              <p className="pt-4">By: <span className="font-bold">{author}</span></p>
               ...
               <div className="flex">
                 <p className="border rounded-full p-2 text-[#328EFF] bg-blue-100">Catagory: {category}</p>
                 <p className="border rounded-full p-2 mx-6 text-[#FFAC33] bg-orange-100	">Rating: {rating}</p>
                 <button className=" border rounded-full p-2 bg-[#23BE0A] text-white">View Details</button>
               </div>
